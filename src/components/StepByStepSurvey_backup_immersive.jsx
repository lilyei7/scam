...
import { 
  ...
  isFaceAPIReady,
  analyzeFaceQuality 
} from '../config/realFaceDetection';
import INEComparison from './INEComparison';
...
export default function StepByStepSurvey() {
  ...
  const handleVisualFaceDetected = (result) => {
    ...
    // Continuar con verificación de vida después de 2 segundos
    setTimeout(() => {
  ...
  const startCamera = async () => {
    try {
      // Verificar requisitos HTTPS primero
      const httpsCheck = checkHTTPSRequirement();
      if (!httpsCheck.supported) {
        updateVerificationResult('error', httpsCheck.message);
        
        if (httpsCheck.reason === 'https_required') {
          // Mostrar instrucciones HTTPS
          updateVerificationResult('error', `
            🔒 Se requiere HTTPS para acceso a cámara.
            
            📱 Opciones:
            1. Usa: https://scamapp.loca.lt
            2. O permite certificado no válido en tu navegador
            
            ⚠️ Esto es requerido por seguridad del navegador
          `);
          return;
        }
      }

      // Verificar soporte de cámara primero (con compatibilidad forzada)
      const browser = await checkCameraSupport();
      console.log(`🌐 Navegador verificado: ${browser.browserName}`, browser);
      
      updateVerificationResult('processing', `🌐 Configurando cámara para ${browser.browserName}...`);

      // Obtener función getUserMedia con compatibilidad Safari
      const getUserMedia = getSafariCompatibleUserMedia();
      
      if (!getUserMedia) {
        throw new Error(`${browser.browserName} no soporta acceso a cámara. Actualiza tu navegador.`);
      }
      ...
      if (browser.isSafari || browser.isIOS) {
        // Safari/iOS necesita configuraciones más específicas
        constraints.video = {
          width: { ideal: 640 },
          height: { ideal: 480 },
          facingMode: { ideal: 'user' },
          frameRate: { ideal: 15, max: 24 }, // Safari funciona mejor con framerates más bajos
        };
        updateVerificationResult('processing', '🍎 Configurando para Safari/iOS...');
      } else if (browser.isFirefox) {
        // Firefox a veces necesita configuraciones diferentes
        constraints.video.frameRate = { ideal: 20, max: 30 };
        updateVerificationResult('processing', '🦊 Configurando para Firefox...');
      } else if (browser.isAndroid) {
        // Android Chrome puede necesitar ajustes
        constraints.video.frameRate = { ideal: 15, max: 25 };
        updateVerificationResult('processing', '🤖 Configurando para Android...');
      }

      updateVerificationResult('processing', `📱 Solicitando acceso a cámara (${browser.browserName})...`);

      // Solicitar acceso a la cámara con reintentos
      let stream;
      let lastError;
      
      // Intentar con la configuración completa
      try {
        stream = await getUserMedia(constraints);
      } catch (error) {
        console.warn('Primer intento falló, probando con configuración básica:', error);
        lastError = error;
        ...
        try {
          ...
          console.warn('Segundo intento falló, probando con facingMode específico:', basicError);
      ...
      if (videoRef.current && stream) {
        ...
        // Configurar video específicamente para cada navegador
        ...
        // Esperar a que el video esté listo con sistema mejorado
        const videoReadyPromise = new Promise((resolve, reject) => {
          const timeout = setTimeout(() => {
            console.warn('⚠️ Timeout de video, pero continuando...');
            // No rechazar, solo continuar
            resolve();
          }, 8000); // Reducir timeout pero no fallar
          
          const checkVideoReady = () => {
            if (videoRef.current && videoRef.current.readyState >= 1) { // Cambiar a >= 1 para ser menos estricto
              clearTimeout(timeout);
              console.log('✅ Video listo con readyState:', videoRef.current.readyState);
              resolve();
            }
          };
          ...
          videoRef.current.onloadstart = () => {
            console.log('📹 Video comenzó a cargar...');
            updateVerificationResult('processing', '📹 Conectando con la cámara...');
          };
        ...
        try {
          ...
          // Intentar reproducir el video de forma más robusta
          updateVerificationResult('processing', '🎬 Activando video...');
          
          try {
            // Intentar reproducir múltiples veces si es necesario
            for (let attempt = 0; attempt < 3; attempt++) {
              try {
                await videoRef.current.play();
                console.log('✅ Video reproduciendo correctamente en intento', attempt + 1);
                break;
              } catch (playError) {
                console.warn(`⚠️ Intento ${attempt + 1} falló:`, playError);
                if (attempt === 2) {
                  // En el último intento, continuar de todas formas
                  console.log('⚡ Continuando sin reproducción automática');
                  updateVerificationResult('warning', '⚡ Cámara lista - Toca la pantalla si no ves el video');
                }
                await new Promise(resolve => setTimeout(resolve, 500)); // Esperar 500ms entre intentos
              }
            }
          } catch (playError) {
            console.warn('⚠️ Video no pudo reproducirse automáticamente:', playError);
            updateVerificationResult('warning', '⚡ Cámara conectada - Toca el video para activar');
          }
          
          updateVerificationResult('processing', `✅ Cámara lista en ${browser.browserName}. Inicializando reconocimiento facial...`);
          console.log('🎥 Cámara lista, iniciando Face-api.js...');

          // Inicializar Face-api.js con verificación mejorada
          console.log('🧠 Intentando inicializar Face-api.js...');
          
          try {
            const faceApiReady = await initializeFaceAPI();
            console.log('🧠 Face-api.js resultado:', faceApiReady);
            
            if (!faceApiReady) {
              console.warn('⚠️ Face-api.js no se inicializó, pero continuando...');
              updateVerificationResult('warning', `⚠️ Sistema básico listo en ${browser.browserName} (detección simplificada)`);
            } else {
              console.log('✅ Face-api.js completamente inicializado');
              updateVerificationResult('success', `✅ Sistema completo listo en ${browser.browserName}`);
            }
          } catch (faceApiError) {
            console.error('❌ Error inicializando Face-api.js:', faceApiError);
            updateVerificationResult('warning', `⚠️ Sistema básico listo en ${browser.browserName} (sin IA avanzada)`);
          }
          ...
          // No mostrar error, sino un estado de advertencia útil
          updateVerificationResult('warning', `⚡ Cámara conectada en ${browser.browserName} - Toca para activar el video`);
        }
      }
    } catch (error) {
      ...
      if (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError') {
        errorMessage = `🚫 Permisos de cámara denegados en ${browser.browserName}. Por favor, permite el acceso y recarga.`;
      } else if (error.name === 'NotFoundError' || error.name === 'DevicesNotFoundError') {
        errorMessage = '📷 No se encontró cámara en tu dispositivo.';
      } else if (error.name === 'NotSupportedError') {
        errorMessage = `❌ ${browser.browserName} no soporta acceso a cámara en este dispositivo.`;
      } else if (error.name === 'NotReadableError') {
        errorMessage = '⚠️ La cámara está siendo usada por otra aplicación.';
      } else if (error.name === 'OverconstrainedError') {
        errorMessage = `⚙️ Configuración de cámara no compatible con ${browser.browserName}. Probando configuración básica...`;
        // Aquí podrías implementar un reintento con configuración más básica
  ...
  const stopCamera = () => {
    ...
    // Limpiar timers de detección automática
    if (autoGestureTimer) {
  ...
  // Función para detectar rostros automáticamente con Face-api.js (mejorada)
  ...
  const startAutomaticGestureSequence = () => {
    if (faceVerificationComplete && !livenessComplete) {
      ...
      // Hacer verificación facial automática
      captureAndVerifyAutomatically();
    }
  };

  // Función para capturar y verificar automáticamente (verificación facial inicial)
  const captureAndVerifyAutomatically = async () => {
    if (!videoRef.current) return;

    setVerificationResult({
      status: 'processing',
      message: 'Capturando y verificando identidad...'
    });
    ...
    try {
      ...
      if (captureResult.success) {
        ...
        setVerificationResult({
          status: 'error',
          message: 'No se pudo capturar el rostro. Reintentando...'
        });
      ...
      setVerificationResult({
        status: 'error',
        message: 'Error en la verificación. Reintentando...'
      });
  ...
  const completeRegistration = () => {
    ...
    // Enviar datos (aquí podrías hacer una llamada a tu API)
    console.log('Datos del cuestionario guardados:', surveyData);
      ...
      <div className="max-w-lg mx-auto">
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8 text-center">
          ...
          <p className="text-gray-600 mb-4">
            Gracias por completar tu registro. Tus datos han sido guardados exitosamente.
          </p>
          
          {/* Mostrar resumen de datos */}
          <div className="bg-gray-50 rounded-lg p-4 mb-6 text-left">
            <h4 className="font-bold text-gray-900 mb-2">Resumen de tu registro:</h4>
            <div className="text-sm text-gray-700 space-y-1">
              {Object.entries(surveyData).map(([key, value]) => {
                if (key === 'timestamp' || key === 'userAgent' || key === 'completed') return null;
                return (
                  <div key={key}>
                    <span className="font-medium">
                      {key === 'situacion_laboral' && 'Situación laboral: '}
                      {key === 'estado_civil' && 'Estado civil: '}
                      {key === 'hijos' && 'Hijos: '}
                      {key === 'documentos_ine' && 'INE: '}
                      {key === 'comprobante_domicilio' && 'Comprobante: '}
                      {key === 'verificacion_persona' && 'Verificación: '}
                    </span>
                    {value}
                  </div>
                );
          ...
          <button
            onClick={resetSurvey}
            className="bg-rose-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-rose-700 transition-colors"
          >
            ...
            <div className="text-center mb-6">
              <div className="relative inline-block">
                ...
                <video
                  ...
                  playsInline
                  muted
                  className="w-80 h-60 bg-gray-900 rounded-lg border-2 border-gray-300"
                  style={{ 
                    display: (getVerificationStatus() === 'success' || getVerificationStatus() === 'warning' || isAutoDetecting || visualDetectionActive) ? 'block' : 'none',
                    objectFit: 'cover'
                  }}
                  onClick={async () => {
                    // Permitir que el usuario active el video tocándolo
                ...
                {(getVerificationStatus() === 'idle' || getVerificationStatus() === 'error') && (
                  <div className="w-80 h-60 bg-gray-100 border-2 border-gray-200 rounded-lg flex items-center justify-center">
                    <div className="text-center">
                      <div className="text-6xl mb-3">📷</div>
                      <p className="text-gray-600 mb-4">Cámara no iniciada</p>
                      <button
                        onClick={startCamera}
